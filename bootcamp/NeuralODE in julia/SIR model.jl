...
dudt2 = Chain(Dense(3,50,tanh),Dense(50,3))
p, st = Lux.setup(rng, dudt2)
prob_neural = NeuralODE(dudt2, tspan, Tsit5(); saveat = tsteps)
...
function callback(state, l; doplot = true)
    ...
    if doplot
        ...
        scatter!(plt, true_data[2, :], label = "Data")
        scatter!(plt, true_data[3, :], label = "Data")
        scatter!(plt, tsteps, pred[1, :]; label = "prediction")
        scatter!(plt, tsteps, pred[2, :]; label = "prediction")
        scatter!(plt, tsteps, pred[3, :]; label = "prediction")
        display(plot(plt))
    end
    return false
end
...
result_neuralode = Optimization.solve(
    optprob, OptimizationOptimisers.Adam(0.1); callback = callback, maxiters = 3000)
